...
import 'package:curation/data/petfood.dart';
import 'package:curation/data/style.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart';
import 'package:url_launcher/url_launcher.dart';
import 'dart:html' as html;

var _url = 'https://aaaawew.imweb.me/shop_view/?idx=149';
...
class _ShowPetfoodScreenState extends State<ShowPetfoodScreen> {
  ...
  Future<dynamic> getPetfood(url) async {
    ...
    if (response.statusCode == 200) {
      print('survey page성공');
      petfoodName = jsonDecode(utf8.decode(response.bodyBytes)).split(',');
      petfoodName.removeAt(0);
    } else {
      print('실패');
    }
  }
  ...
  void initState() {
    super.initState();
    getPetfood('http://3.17.203.194:8000/app1/filter/').then((value) {
      setState(() {
        for (var i = 0; i < petfoodName.length; i++) {
          for (var j = 0; j < data.length; j++) {
            if (petfoodName[i] == data[j]['name']) {
              petfood.add(data[j]);
            }
          }
        }
        ...
        Positioned(
          ...
          child: InkWell(
              ...
              onTap: () {
                setState(() {
                  html.window.open(
                      'https://aaaawew.imweb.me/shop_view/?idx=149', '_self');
                  // launch('https://aaaawew.imweb.me/shop_view/?idx=149',
                  //     forceWebView: true);
                  //이건 페이지 전환
                  // showDialog(
                  //   context: context,
                  ...
                  //           postUserData(
                  //               'http://3.17.203.194:8000/app1/sign-in/');
                  //         },
                  //         child: Text(
                  //           '확인',
                  //         ),
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          fontSize:
                              petfood[index]['name'].toString().length > 11
                                  ? 17
                                  : 19,
                          fontWeight: FontWeight.w700,
                        ),
